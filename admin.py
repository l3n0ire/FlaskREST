...
def removeCourse(courseCode):
    collection.remove({"courseCode":courseCode})
...
def subscribe(user,courseCode):
    collection.update_one(
        {"courseCode":courseCode},
        {"$push":{"subscribers":{"name":user, "id":3} } } )
...
def unsubscribe(user,courseCode):
    collection.update_one(
        {"courseCode":courseCode},
        {"$pull":{"subscribers":{"name":user, "id":3} } } )
...
def getCourse(courseCode):
    if courseCode =="ALL":
        ...
        return collection.find({"courseCode":courseCode})

def getCoursesAsString():
